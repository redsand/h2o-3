...
//      params._lambda_search = true;
  ...
  public double[] convert2DBetaTo1D(double[][] multBeta, int nclass, int ncoeffPClass) {
    double[] beta1D = new double[nclass*ncoeffPClass];
    for (int classInd = 0; classInd < nclass; classInd++) {
      for (int coeffInd = 0; coeffInd < ncoeffPClass; coeffInd++)  {
        int destInd = classInd*ncoeffPClass;
        System.arraycopy(multBeta[classInd], 0, beta1D, destInd, ncoeffPClass);
      }
    }
    return beta1D;
  }
...
/*    double[][] multinomialBetas = new double[nclass][ncoeffPClass];
  ...
  /* 2. Verify ADMM implementation considering all predictors.  I am checking the weight obtains from octave and
   * compare the weight update from java for one iterations.  No regularization is applied in this case.
   */
  @Test
  public void testMultinomialWeightUpdateOctaveCheck() {
    String filename = "smalldata/glm_test/multinomial_3_cols_num_3_class_20Rows.csv"; // all numeric columns
    double[] beta1Iter = new double[] {8.68575858E-01, 5.12097376E-01, 1.28991945E-01, 2.12836461E-02, -2.77665832E-02,
            1.20357519E-01, 5.08333331E-02, 8.74527787E-02, -8.40809275E-01, -6.32454896E-01, -1.79825278E-01,
            3.63372093E-02};
    compareWeightUpdatesOctave(filename, beta1Iter, 1, Solver.IRLSM_SPEEDUP);  // check weight after 1 iteration
  }

  public void compareWeightUpdatesOctave(String filename, double[] beta, int numIter, Solver solver) {
    Scope.enter();
    double threshold = 1e-6;
    try {
      Frame fr = parse_test_file(filename);
      ...
      Scope.track_generic(model);
      double[] multinomialBeta = convert2DBetaTo1D(model._output.get_global_beta_multinomial(), nclass, model._output.get_global_beta_multinomial()[0].length);
      compareGLMCoeffs(multinomialBeta, beta, 1e-6);
    } finally {
      Scope.exit();
    }
  }
  ...
  public void testMultinomialActiveCols() {
    String filename = "smalldata/glm_test/multinomial_3_cols_num_3_class_20Rows.csv"; // all numeric columns
    compareWeightUpdates2Solvers(filename, 100, Solver.IRLSM_SPEEDUP, Solver.IRLSM_SPEEDUP2,
  ...
  public void testMultinomialActiveColsEnums() {
    ...
    compareWeightUpdates2Solvers(filename, 100, Solver.IRLSM_SPEEDUP, Solver.IRLSM,
            0.0001, 0.5);  // compare L1 and L2 regularization
  }
  ...
  public void testMultinomialADMMActiveCols() {
    ...
    try {
      ...
      // check both algos with no regularization, MoreThuente LS is used in this case and no ADMM.
      GLMModel noregspeedup2 = checkADMM(fr, test, 0,0, true, Solver.IRLSM_SPEEDUP2);
      Scope.track_generic(noregspeedup2);
      GLMModel noregspeedup = checkADMM(fr, test, 0, 0, true, Solver.IRLSM_SPEEDUP);
      Scope.track_generic(noregspeedup);
      checkLoglossMeanError(noregspeedup2, noregspeedup, threshold);
      // compare multinomial coefficients for no regularization, active columns in this case include all predictors
      TestUtil.checkArrays(noregspeedup2.beta(), noregspeedup.beta(), threshold);
